...
- **Vision** — Limelights on the real robot, PhotonVision sim in SIM (with PR #1767 PNP trig-solve for single tags), distance-weighted standard deviations
...
- **REAL** — five Limelights. Names in `LIMELIGHT_NAMES` (`limelight-br`, `limelight-bl`, …) must match each camera's NetworkTables name (set in the LL web UI). Filtering and MegaTag1/MegaTag2 selection live in `VisionIOLimelight`.
- **SIM** — four simulated PhotonVision cameras (`VisionIOPhotonVisionSim`). Names in `PHOTON_CAMERA_NAMES`; per-camera mounts in `PHOTON_CAMERA_TRANSFORMS` (`Transform3d` from robot center — **tune against your CAD** before trusting trig-solve distances). The sim is fed from MapleSim ground truth in `updateSimulation`. Multi-tag observations use coprocessor PNP (MT1 analog); single-tag observations use `PNP_DISTANCE_TRIG_SOLVE` from [PhotonVision PR #1767](https://github.com/PhotonVision/photonvision/pull/1767) — the MT2 analog that fuses gyro heading with tag distance so rotation comes from the gyro.
- **REPLAY** — both name sets are registered as `VisionIONoop` so logs from either source replay correctly. Names not in the log produce no observation and contribute nothing.

`VisionIOPhotonVision` (real PhotonVision) is also in the tree — swap it into the REAL case if you switch hardware.
