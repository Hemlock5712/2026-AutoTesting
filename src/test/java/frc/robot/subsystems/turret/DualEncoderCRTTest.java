 ...
 *
 * <p>Tests the pure calculation logic with 110:21 and 110:22 gear ratios. Encoder values are
 * computed as (mechanismPosition * 110/21) mod 1 and (mechanismPosition * 110/22) mod 1. Output is
 * shifted to turret range [-0.25, 0.75).
 */
class DualEncoderCRTTest {
  ...
  class BasicCRTCalculation {
    ...
    void quarterRotation_returnsCorrectPosition() {
      // At mechanism position 0.25: e1 = 0.25*110/21 mod 1 = 0.31, e2 = 0.25*110/22 mod 1 = 0.25
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.3095, 0.25);
      assertEquals(0.25, result, TOLERANCE);
    }

    @Test
    void halfRotation_returnsCorrectPosition() {
      // At mechanism position 0.5: e1 = 0.619, e2 = 0.5
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.619, 0.5);
      assertEquals(0.5, result, TOLERANCE);
    }

    @Test
    void slightlyPastHalfRotation_returnsCorrectPosition() {
      // At mechanism position 0.546: e1 = 0.86, e2 = 0.73
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.86, 0.73);
      assertEquals(0.546, result, TOLERANCE);
    }

    @Test
    void threeQuarterRotation_shiftsToNegativeQuarter() {
      // At mechanism position 0.75: e1 = 0.929, e2 = 0.75 -> shifts to -0.25
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.9286, 0.75);
      assertEquals(-0.25, result, TOLERANCE);
    }

    @Test
    void oneEighthRotation_returnsCorrectPosition() {
      // At mechanism position 0.125: e1 = 0.655, e2 = 0.625
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.6548, 0.625);
  ...
  class RangeShift {
    ...
    void boundaryAt075_shiftsToNegativeQuarter() {
      // At -90 deg (reverse limit): e1=0.929, e2=0.75 -> mech=0.75 -> shift to -0.25
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.9286, 0.75);
      assertEquals(-0.25, result, TOLERANCE);
    }
    ...
    void justAbove075_shiftsToNegative() {
      // Mechanism 0.8: e1=0.19, e2=0 -> shift to -0.2
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.1905, 0.0);
      assertEquals(-0.2, result, TOLERANCE);
    }

    @Test
    void justBelow075_staysPositive() {
      // Mechanism 0.74: e1=0.876, e2=0.7
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.8762, 0.7);
      assertEquals(0.74, result, TOLERANCE);
    }

    @Test
    void nearOne_shiftsToNearNegativeQuarter() {
      // Mechanism 0.99 (= -0.01): e1=0.186, e2=0.95
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.1857, 0.95);
  ...
  class EncoderWrap {
    ...
    void rawValuesOutsideZeroOne_wrapCorrectly() {
      // Mechanism -0.2: e1=0.19, e2=0. Use raw 1.19, 2.0 to wrap to these
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(1.1905, 2.0);
      assertEquals(-0.2, result, TOLERANCE);
    }

    @Test
    void negativeRawValues_wrapCorrectly() {
      // Mechanism 0.1: e1=0.524, e2=0.5. Use raw -0.476, -0.5
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(-0.476, -0.5);
  ...
  class ConsistencyCheck {
    ...
    void slightlyInconsistentReadings_returnNaN() {
      // e1=0, e2=0.1 - inconsistent pair
      double result = DualEncoderCRT.calculateMechanismPositionFromEncoders(0.0, 0.1);
      assertTrue(Double.isNaN(result));
    }
