...
abstract class ExtGeometry<T> implements Tunable, Supplier<T> {
    ...
    /**
     * Saves a new value to the container, with all flipped variations. This method
     * is invoked in the
     * constructor, and can also be utilized by {@link #initTunable(TunableTable)}.
     *
     * <p>
     * This method should implement setting the values of {@link #original},
     * {@link #overWidth},
    ...
    /**
     * Gets the value for the robot's current alliance. Flipping behavior is
     * inherited from {@link
     * FieldInfo#symmetryType()}.
     */
    @Override
    public T get() {
        return get(false);
    ...
    public T get(boolean flipWidth) {
        return get(FieldInfo.shouldFlip(), flipWidth);
    }

    /**
     * Gets the value for the specified alliance. Flipping behavior is inherited
     * from {@link
     * FieldInfo#symmetryType()}.
     *
