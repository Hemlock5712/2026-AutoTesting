package frc.robot.simlib.motorsims;
...
public class MapleMotorSim {
  ...
  public void update(Time dt) {
    // Substep to keep explicit Euler stable against the rotor-back-EMF time constant. For an
    // FRC swerve steer (Kraken, gearing ~26, load MOI 0.01 kg·m²) the natural time constant is
    // ≈0.2 ms; the outer 4 ms sub-tick alone is wildly unstable (each step multiplies velocity
    // by ~−19 and overflows in ~100 ticks). Splitting into NUM_SUBSTEPS keeps the inner step
    // well under 2τ for any FRC motor + reasonable swerve gearing. Cheap: each substep is a
    // few muls and a clamp; even at 4 motors × 5 sub-ticks × 100 substeps = 2 000 substeps per
    // 20 ms period, we are well under the budget.
    final int NUM_SUBSTEPS = 100;
    Time subDt = dt.div(NUM_SUBSTEPS);
    for (int i = 0; i < NUM_SUBSTEPS; i++) {
      ...
      this.state.step(
          configs.calculateTorque(statorCurrent), configs.friction, configs.loadMOI, subDt);
    }
